...
import 'notification_diolog_box.dart';
...
class PushNotificationSystem{
  ...
  Future initializeCloudMessaging(BuildContext context) async{
    //1 Terminated
    //When the app is closed and opened directly from push notification
    ...
    //2 Foreground
    //When the app is opened and recieves a push notification
      ...
      if(event.snapshot.value ==  "waiting"  || event.snapshot.value == firebaseAuth.currentUser!.uid){
        FirebaseDatabase.instance.ref().child("All Ride Requests").child(userRideRequestId).once().then((snapData) {
          if(snapData.snapshot.value != null){
            ...
            Fluttertoast.showToast(msg: "This Ride Request Id does not exists");
          }
        });
      }
      else{
